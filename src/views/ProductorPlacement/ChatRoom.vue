<template>
  <div style="padding: 10px; margin-bottom: 50px">
    <div class="chatContainer">
      <el-card style="height: 60vh; color: #333;margin-bottom: 20px">
        <div style="padding-bottom: 10px; border-bottom: 1px solid #ccc">
          在线用户<span style="font-size: 12px">（点击用户名开始聊天）</span>
        </div>
        <div style="padding: 10px 0" v-for="user in users" :key="user.username">
          <span @click="chatUser = user.username" style="cursor: pointer">{{
            user.username
          }}</span>
          <span
...
export default {
  ...
  methods: {
    ...
    init() {
      this.user = localStorage.getItem("rsaUserInfo")
        ? JSON.parse(localStorage.getItem("rsaUserInfo"))
        : {};
      ...
      if (typeof WebSocket == "undefined") {
        ...
        let socketUrl = "wss://innerpurity.top:2599/imserver/" + username;
        if (socket != null) {
          socket.close();
          socket = null;
        }
